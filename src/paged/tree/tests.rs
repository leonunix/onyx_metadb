...
fn v(n: u8) -> L2pValue {
    let mut x = [n; 36];
    // Onyx invariant for v3 compact encoding: unit_original_size must
    ...
    // seq trailer: small monotonic u64 so the per-leaf u32 seq_delta
    // encoding doesn't overflow when distinct `n` values land in the
    // same leaf.
