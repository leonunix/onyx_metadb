...
### 4.4 leaf-rc-suppress 对称性

- take_snapshot 之后，current 对 lba X 的写：leaf shared → decref 抑制
- drop_snapshot 时：对 "snap 有 current 已不同" 的 lba，补发 decref

组合起来：**不管是否有 snapshot 干扰**，一次 `take → N 次 writes → drop` 之后，
每个 pba 的 refcount 应当等于 "没做 snapshot 直接做那 N 次 writes" 的终值。
