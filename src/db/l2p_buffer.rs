//! 4-slot TXG ring buffer for L2P updates.
//!
//! Commits stamped to TXG `n` insert into slot `n & (TXG_SIZE - 1)`. The
//! background [`crate::db::txg_sync::TxgSyncThread`] drains the slot
//! corresponding to the currently-Syncing TXG into the on-disk paged
//! radix tree; `flush_with_gate`'s inline path
//! ([`crate::db::Db::force_compact_l2p_buffers`]) drains every slot
//! when `txg_threads_enabled = false`.
//!
//! ## Slot semantics
//!
...
//! ## Lookup ordering
//!
//! [`L2pBuffer::lookup_for_open_txg`] walks slots in newest-first order:
...
impl L2pBuffer {
    ...
    /// Insert into the slot indexed by `txg & (TXG_SIZE - 1)`. Caller
    /// must hold a [`crate::txg::TxgGuard`] for `txg`, ensuring the
    ...
    /// `(open_txg-1) & 3` → `(open_txg-2) & 3`. The 4th slot is always
    ...
    pub fn lookup_for_open_txg(&self, open_txg: Txg, lba: Lba) -> BufferLookup {
        // `open_txg` is at least 1 by construction (`TxgStateMachine`
    ...
    /// `TxgSyncThread`) must have already promoted the slot to Syncing
    ...
    /// concurrently. Used when `txg_threads_enabled = false` and by
    ...
    /// Concurrent callers (sync thread draining its TXG slot vs. an
...
mod tests {
    ...
    fn lookup_absent_on_empty() {
        ...
        assert_eq!(b.lookup_for_open_txg(1, 42), BufferLookup::Absent);
    }
    ...
    fn lookup_for_open_txg_skips_wraparound_stale_slot() {
        let b = L2pBuffer::new(0);
        // Plant in slot 0 with TXG=4 (stale from open=1's perspective).
        b.insert_at_txg(4, 42, val(9), 400);
        b.insert_at_txg(1, 42, val(1), 101);
        // open=4 walks slot 0 (txg=4), slot 3, slot 2; should see val(9).
    ...
    fn lookup_during_bootstrap_does_not_underflow() {
        // open_txg=1 visits slot 1, slot 0; no underflow.
        let b = L2pBuffer::new(0);
        b.insert_at_txg(1, 42, val(1), 101);
        assert_eq!(b.lookup_for_open_txg(1, 42), BufferLookup::Present(val(1)));
    ...
    fn drain_all_slots_merges_and_picks_highest_lsn() {
        let b = L2pBuffer::new(0);
        b.insert_at_txg(1, 42, val(1), 101);
        b.insert_at_txg(2, 42, val(2), 202);
        b.insert_at_txg(3, 99, val(9), 303);
